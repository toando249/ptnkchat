  ...
  if (data.attachments) {
    if (data.attachments[0]) {
      ...
      if (type === 'fallback') {
        ...
        const res = await phin({
          url: `https://link1s.com/api?api=80cf501d5da3029daf180ea40277292339c053f1&url=${data.attachments[0]}&format=text`,
          method: 'get',
          parse: 'json',
        });
        console.log(res.body)
        await fb.sendTextButtons(receiver, JSON.stringify(res.body), false, true, false, false, true);
    ...
    for (let i = 1; i < data.attachments.length; i++) {
      ...
      if (type === 'image' || type === 'video') {
        const res = await phin({
          url: `https://link1s.com/api?api=80cf501d5da3029daf180ea40277292339c053f1&url=${data.attachments[i]}&format=text`,
          method: 'get',
          parse: 'json',
        });
        console.log(res.body)
