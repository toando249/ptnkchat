  ...
  if (data.attachments) {
    if (data.attachments[0]) {
      ...
      if (type === 'fallback') {
        ...
        await fb.sendTextMessage(sender, receiver, data.attachments[0].payload.url, true);
      } else {
    ...
    for (let i = 1; i < data.attachments.length; i++) {
      ...
      if (type === 'image' || type === 'video' || type === 'audio' || type === 'file') {
        await fb.sendTextMessage(sender, receiver, data.attachments[i].payload.url, true);
      }
    }
  } else {
